...
use ring::{aead, agreement, digest, error, hkdf, hmac, rand};
use x25519_dalek;
use elliptic_curve;
use p256;
...
const MAX_PRIVATE_KEY_LEN: usize = 32;
const MAX_PUBLIC_KEY_LEN: usize = 1 + (2 * 32);
...
pub enum KemId {
    ...
    P256Sha256 = 0x0010,
    /// DHKEM(P-384, HKDF-SHA384) -- unimplemented
    P384Sha384 = 0x0011,
...
/// KeyPair
pub struct KeyPair {
    secret_key: [u8; MAX_PRIVATE_KEY_LEN],
...
impl Kem {
    ...
    pub fn private_key_length(&self) -> usize {
        32 // we currently implement P-256 and X25519
    }

    /// Write the serialized public key bytes into output buffer `buf`
    fn public_key_bytes(&self, secret_key: &[u8], buf: &mut [u8]) -> Result<(), error::Unspecified> {
        ...
        match self.kem_id {
            KemId::P256Sha256 => {
                let secret = p256::SecretKey::from_be_bytes(secret_key).unwrap();
    ...
    pub fn generate_key_pair(
        ...
        if let Some(rng) = rng {
            match self.kem_id {
                KemId::P256Sha256 => {
                    secret_key = elliptic_curve::SecretKey::random(rng).to_be_bytes().as_slice().try_into().expect("rip");
                },
                KemId::X25519Sha256 => {
                    secret_key = x25519_dalek::StaticSecret::new(rng).to_bytes();
                },
                _ => panic!("Unimplemented!")
            }
        } else {
            match self.kem_id {
                KemId::P256Sha256 => {
                    secret_key = elliptic_curve::SecretKey::random(rand::SystemRandom::new()).to_be_bytes().as_slice().try_into().expect("rip");
                },
                KemId::X25519Sha256 => {
                    secret_key = x25519_dalek::StaticSecret::new(rand::SystemRandom::new()).to_bytes();
                },
                _ => panic!("Unimplemented!")
            }
        };
    ...
    pub fn derive_key_pair(&self, seed: &[u8]) -> Result<KeyPair, error::Unspecified> {
        ...
        match self.kem_id {
            KemId::X25519Sha256 => {
                ...
                seed_array: [u8; 32] = seed.try_into().expect("rip");
                let secret_key = x25519_dalek::StaticSecret::from(seed_array).to_bytes();
                self.public_key_bytes(&secret_key, &mut public_key[..self.public_key_length()])?;
                Ok(KeyPair {
                    secret_key,
                    public_key,
            ...
            KemId::P256Sha256 => {
                for counter in 0u8..=255 {
                    ...
                    let secret_key = elliptic_curve::SecretKey::from_be_bytes(seed).to_be_bytes().as_slice().try_into().expect("rip");
                    self.public_key_bytes(&secret_key, &mut public_key[..self.public_key_length()])?;
                    return Ok(KeyPair {
                        secret_key: k,
                        public_key,
    ...
    fn dh_kex(
        ...
        match self.kem_id {
            KemId::P256Sha256 => {
                let secret_key = p256::SecretKey::from_be_bytes(&keypair.secret_key).unwrap();
                let peer_public_key = p256::PublicKey::from_sec1_bytes(public_key_receiver).unwrap();
                let shared = elliptic_curve::ecdh::diffie_hellman(secret_key, peer_public_key);
                shared_secret.copy_from_slice(shared.as_bytes());
            },
            KemId::X25519Sha256 => {
                let secret_key = x25519_dalek::StaticSecret::from(keypair.secret_key);
                let peer_public_key = x25519_dalek::PublicKey::from(public_key_receiver);
                let shared = secret_key.diffie_hellman(&peer_public_key);
                shared_secret.copy_from_slice(shared.as_bytes());
            },
            _ => panic!("rip"),
        }
        encapped_key.copy_from_slice(keypair.public_key_bytes());
        Ok(())
    }
    ...
    fn decap(
        ...
        match self.kem_id {
            KemId::P256Sha256 => {
                let secret_key = p256::SecretKey::from_be_bytes(&keypair.secret_key).unwrap();
                let public_key_ephemeral = p256::PublicKey::from_sec1_bytes(encapped_key).unwrap();
                let shared = elliptic_curve::ecdh::diffie_hellman(secret_key, public_key_ephemeral);
                kex_result.copy_from_slice(shared.as_bytes());
            },
            KemId::X25519Sha256 => {
                let secret_key = x25519_dalek::StaticSecret::from(keypair.secret_key);
                let public_key_ephemeral = x25519_dalek::PublicKey::from(encapped_key);
                let shared = secret_key.diffie_hellman(&public_key_ephemeral);
                kex_result.copy_from_slice(shared.as_bytes());
            },
            _ => panic!("rip"),
        }
